...
Popup_ContaVisible() {
    ...
    if !campoConta
        campoConta := MV_FindControlAtPoint(
            MV_WIN_FFCV_ANY, "Edit",
            MV_POPUP_CAMPO_CONTA_X, MV_POPUP_CAMPO_CONTA_Y, 50, "Edit")

    if !campoConta
        campoConta := MV_FindControlAtPoint(
            MV_WIN_FFCV_ANY, "ComboBox",
            MV_POPUP_DROPDOWN_TIPO_X, MV_POPUP_DROPDOWN_TIPO_Y, 40, "ComboBox")

    if !campoConta
        campoConta := MV_FindControlAtPoint(
            MV_WIN_FFCV_ANY, "ComboBox",
            MV_POPUP_DROPDOWN_SUB_TIPO_X, MV_POPUP_DROPDOWN_SUB_TIPO_Y, 40, "ComboBox")
...
Popup_DismissActiveModal() {
    ...
    try {
        ...
        if !MV_Poll(() => MV_FirstControlByClass(popup, MV_MODAL_OK_CLASS) != 0,
